...
- **框架**: Svelte 5 + SvelteKit 2+ (使用 runes 模式)
- **构建工具**: Vite 6.0+
- **UI组件库**: shadcn-svelte + Tailwind CSS 3.4+
- **图标系统**: @iconify/svelte (支持 Tabler Icons)
...
Stellar 采用了**现代化玻璃质感设计系统 (Glass Morphism)**，融合了当代 UI 设计的最佳实践：

#### 设计理念
- **玻璃质感 (Glass Morphism)**: 使用透明度、背景模糊和微妙阴影创造深度感
- **渐变色彩**: 从蓝色到紫色的品牌渐变，营造科技感和专业性
- **响应式设计**: 完美适配桌面端、平板和移动设备
- **暗色/亮色主题**: 支持自动切换和手动选择主题模式

#### 视觉特色
- **背景效果**: 渐变背景 + 动态光斑装饰
- **卡片设计**: 半透明背景 + 背景模糊 (`backdrop-blur-xl`)
- **现代图标**: 使用 Tabler Icons 提供的专业图标系统
- **动画交互**: 丰富的悬停效果和过渡动画
- **色彩系统**: 基于 Tailwind CSS 的一致性色彩方案

#### 组件特性
- **侧边栏导航**: 可折叠设计，支持图标和描述显示
- **统计卡片**: 带有渐变图标和动画效果的数据展示
- **表单界面**: 现代化表单设计，支持图标前缀和状态反馈
- **通知系统**: 优雅的消息提示和状态反馈机制
...
- **热重载**: 前端和后端代码修改自动重新加载
- **端口管理**: 自动检测并处理端口冲突 (8090, 5173)
- **进程管理**: 智能进程启动和停止，PID 文件管理
- **实时监控**: 服务状态检查和错误报告
- **现代化 UI**: 玻璃质感设计，响应式布局
- **TypeScript**: 全栈类型安全支持
...
#### 路由系统
- 路由组织：`routes/(app)/` 应用路由，`routes/(auth)/` 认证路由
- 路由跳转：使用 `goto('/path')` 而非路由组语法
- 布局继承：自动继承父级 `+layout.svelte` 样式和逻辑
...
// 图标使用
<Icon icon="tabler:home" width={20} />

// 响应式卡片
<Card class="hover:shadow-lg transition-all bg-white/80 backdrop-blur-sm">
  <CardHeader>
...
- 玻璃质感效果：`bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl`
