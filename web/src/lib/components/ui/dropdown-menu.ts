...
export function DropdownMenu(node: HTMLDivElement, props: { class?: string } = {}) {
	node.className = cn('relative inline-block text-left', props.class);

	return {
		update(newProps: { class?: string }) {
			node.className = cn('relative inline-block text-left', newProps.class);
		}
	};
}
...
export function DropdownMenuContent(
	node: HTMLDivElement,
	props: { class?: string; align?: 'start' | 'end' } = {}
) {
	node.className = cn(
