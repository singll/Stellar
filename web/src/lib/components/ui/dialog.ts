...
export function Dialog(node: HTMLDivElement, props: { class?: string; open?: boolean } = {}) {
	...
	node.className = cn('relative z-50', props.class);
	...
	return {
		update(newProps: { class?: string; open?: boolean }) {
			node.className = cn('relative z-50', newProps.class);
			if (newProps.open) {
				node.style.display = 'block';
			} else {
				node.style.display = 'none';
...
export function DialogContent(node: HTMLDivElement, props: { class?: string } = {}) {
	node.className = cn('fixed inset-0 z-50 flex items-center justify-center', props.class);
	...
	return {
		update(newProps: { class?: string }) {
			node.className = cn('fixed inset-0 z-50 flex items-center justify-center', newProps.class);
		}
	};
}

export function DialogHeader(node: HTMLDivElement, props: { class?: string } = {}) {
	node.className = cn('flex flex-col space-y-1.5 text-center sm:text-left', props.class);

	return {
		update(newProps: { class?: string }) {
			node.className = cn('flex flex-col space-y-1.5 text-center sm:text-left', newProps.class);
		}
	};
}

export function DialogTitle(node: HTMLHeadingElement, props: { class?: string } = {}) {
	node.className = cn('text-lg font-semibold leading-none tracking-tight', props.class);

	return {
		update(newProps: { class?: string }) {
			node.className = cn('text-lg font-semibold leading-none tracking-tight', newProps.class);
		}
	};
}
...
export function DialogDescription(node: HTMLParagraphElement, props: { class?: string } = {}) {
	...
	return {
		update(newProps: { class?: string }) {
			node.className = cn('text-sm text-muted-foreground', newProps.class);
		}
	};
}
