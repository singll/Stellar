...
<script lang="ts">
	...
	// Store 订阅
	const tasks = portScanStore.tasks;
	const loading = portScanStore.loading;
	const pagination = portScanStore.pagination;
	const taskSummary = portScanStore.taskSummary;
	const runningTasks = portScanStore.runningTasks;
...
<div class="container mx-auto px-4 py-6">
	...
	<div class="bg-white rounded-lg shadow-sm border p-4 mb-6">
		<div class="flex flex-wrap gap-4 items-center">
			<SearchInput bind:value={searchTerm} placeholder="搜索目标地址..." on:search={applyFilters} />
		...
		{#if showFilters}
			<div class="mt-4 pt-4 border-t">
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					<div>
						<label for="created-date-filter" class="block text-sm font-medium text-gray-700 mb-1">创建时间</label>
						<input
					...
					<div>
						<label for="end-date-filter" class="block text-sm font-medium text-gray-700 mb-1">结束时间</label>
						<input
							id="end-date-filter"
							type="date"
							class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
					...
					<div>
						<label for="creator-filter" class="block text-sm font-medium text-gray-700 mb-1">创建者</label>
						<input
							id="creator-filter"
							type="text"
							placeholder="输入创建者..."
								...
								<td class="px-4 py-3">
									...
									{#if task.endTime}
										<div class="text-xs text-gray-500">
											耗时: {task.startTime ? formatDuration(task.startTime, task.endTime) : '-'}
										</div>
									{/if}
								</td>
								<td class="px-4 py-3">
