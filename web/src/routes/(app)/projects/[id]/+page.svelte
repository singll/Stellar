<script lang="ts">
  ...
  import { ArrowLeft, Edit, Settings, Share, Download, Play, Pause, MoreHorizontal, Users, Activity, Target, Shield } from 'lucide-svelte';
    ...
    <div class="flex items-start justify-between">
      ...
      <div class="flex items-center gap-2">
        {#if project.scan_status === 'running'}
          <Button variant="outline" onclick={handlePauseScan} disabled={loading} class="flex items-center gap-2">
            <Pause class="h-4 w-4" />
            暂停扫描
          </Button>
        {:else}
        ...
        <Button variant="outline" href="/projects/{project.id}/edit" class="flex items-center gap-2">
          <Edit class="h-4 w-4" />
          编辑
        </Button>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <Card>
        ...
        <CardContent>
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Button variant="outline" href="/projects/{project.id}/assets" class="flex items-center gap-2 h-auto py-4 flex-col">
              <Target class="h-6 w-6" />
              <span class="text-sm">资产管理</span>
            </Button>
            
            <Button variant="outline" href="/projects/{project.id}/vulnerabilities" class="flex items-center gap-2 h-auto py-4 flex-col">
              <Shield class="h-6 w-6" />
              <span class="text-sm">漏洞报告</span>
            </Button>
            
            <Button variant="outline" href="/projects/{project.id}/tasks" class="flex items-center gap-2 h-auto py-4 flex-col">
              <Activity class="h-6 w-6" />
              <span class="text-sm">任务管理</span>
            </Button>
            
            <Button variant="outline" onclick={() => handleExport('json')} class="flex items-center gap-2 h-auto py-4 flex-col">
    ...
    <div class="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle class="flex items-center justify-between">
            项目成员
            <Button variant="outline" size="sm" href="/projects/{project.id}/members">
              管理
            </Button>
          </CardTitle>
        </CardHeader>
          ...
          {:else if activities.data}
            <div class="space-y-3">
              ...
              {#if activities.data.length > 5}
                <Button variant="outline" size="sm" href="/projects/{project.id}/activities" class="w-full">
                  查看更多活动
                </Button>
              {/if}
            </div>
      ...
      <Card>
        ...
        <CardContent>
          <Button variant="outline" onclick={handleDelete} class="w-full text-red-600 hover:text-red-800 border-red-200 hover:border-red-300">
            删除项目
          </Button>
        </CardContent>
      </Card>
