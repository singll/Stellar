...
func (s *TaskScheduler) Start() error {
	...
	if err := s.loadScheduleRules(); err != nil {
		logger.Error("Start load schedule rules failed", map[string]interface{}{"error": err})
		return pkgerrors.WrapError(err, pkgerrors.CodeConfigError, "加载调度规则失败", 500)
	}
	...
	logger.Info("Start task scheduler started", map[string]interface{}{"rulesCount": len(s.scheduleRules)})
	return nil
}
...
func (s *TaskScheduler) CreateScheduleRule(rule *models.TaskScheduleRule) error {
	...
	if err := s.calculateNextRunTime(rule); err != nil {
		logger.Error("CreateScheduleRule calculate next run time failed", map[string]interface{}{"rule": rule, "error": err})
		return pkgerrors.WrapError(err, pkgerrors.CodeValidationFailed, "计算下次执行时间失败", 400)
	}

	// 保存到数据库
	_, err := s.db.Collection("task_schedule_rules").InsertOne(s.ctx, rule)
	if err != nil {
		logger.Error("CreateScheduleRule save rule failed", map[string]interface{}{"rule": rule, "error": err})
		return pkgerrors.WrapDatabaseError(err, "保存调度规则")
	}
	...
	if rule.Enabled {
		if err := s.addCronJob(rule); err != nil {
			logger.Error("CreateScheduleRule add cron job failed", map[string]interface{}{"ruleID": rule.ID.Hex(), "error": err})
		}
	}
...
func (s *TaskScheduler) UpdateScheduleRule(ruleID string, updates *models.TaskScheduleRule) error {
	...
	if err != nil {
		logger.Error("UpdateScheduleRule invalid ruleID", map[string]interface{}{"ruleID": ruleID, "error": err})
		return pkgerrors.NewAppErrorWithCause(pkgerrors.CodeBadRequest, "无效的规则ID", 400, err)
	}

	// 获取原规则
	rule, exists := s.scheduleRules[ruleID]
	if !exists {
		logger.Error("UpdateScheduleRule rule not found", map[string]interface{}{"ruleID": ruleID})
		return pkgerrors.NewAppError(pkgerrors.CodeNotFound, "调度规则不存在", 404)
	}
	...
	if updates.CronExpression != "" && updates.CronExpression != rule.CronExpression {
		...
		if err := s.calculateNextRunTime(rule); err != nil {
			logger.Error("UpdateScheduleRule calculate next run time failed", map[string]interface{}{"ruleID": ruleID, "error": err})
			return pkgerrors.WrapError(err, pkgerrors.CodeValidationFailed, "计算下次执行时间失败", 400)
		}
	}
	...
	if err != nil {
		logger.Error("UpdateScheduleRule update rule failed", map[string]interface{}{"ruleID": ruleID, "error": err})
		return pkgerrors.WrapDatabaseError(err, "更新调度规则")
	}
	...
	if rule.Enabled {
		if err := s.addCronJob(rule); err != nil {
			logger.Error("UpdateScheduleRule add cron job failed", map[string]interface{}{"ruleID": ruleID, "error": err})
		}
	}
