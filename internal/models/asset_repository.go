...
func (r *AssetRepository) CreateAsset(ctx context.Context, assetType AssetType, asset interface{}) (primitive.ObjectID, error) {
	...
	if err != nil {
		logger.Error("CreateAsset failed", map[string]interface{}{"assetType": assetType, "error": err})
		return primitive.NilObjectID, pkgerrors.NewAppErrorWithCause(pkgerrors.CodeInternalError, "CreateAsset failed", 500, err)
	}
	id, ok := result.InsertedID.(primitive.ObjectID)
	if !ok {
		logger.Error("CreateAsset returned invalid ID type", map[string]interface{}{"assetType": assetType})
		return primitive.NilObjectID, pkgerrors.NewAppError(pkgerrors.CodeInternalError, "插入资产返回ID类型错误", 500)
	}
	return id, nil
}
...
func (r *AssetRepository) FindAssetByID(ctx context.Context, assetType AssetType, id primitive.ObjectID, result interface{}) error {
	collName := AssetCollection(assetType)
	err := r.db.Collection(collName).FindOne(ctx, bson.M{"_id": id}).Decode(result)
	if err != nil {
		logger.Error("FindAssetByID failed", map[string]interface{}{"assetType": assetType, "id": id.Hex(), "error": err})
		if err == mongo.ErrNoDocuments {
			return pkgerrors.NewNotFoundError("asset not found")
		}
		return pkgerrors.NewAppErrorWithCause(pkgerrors.CodeInternalError, "FindAssetByID failed", 500, err)
	}
	return nil
}
...
func (r *AssetRepository) UpdateAsset(ctx context.Context, assetType AssetType, id primitive.ObjectID, update bson.M) error {
	...
	if err != nil {
		logger.Error("UpdateAsset failed", map[string]interface{}{"assetType": assetType, "id": id.Hex(), "error": err})
		return pkgerrors.NewAppErrorWithCause(pkgerrors.CodeInternalError, "UpdateAsset failed", 500, err)
	}
	return nil
}
...
func (r *AssetRepository) DeleteAsset(ctx context.Context, assetType AssetType, id primitive.ObjectID) error {
	...
	if err != nil {
		logger.Error("DeleteAsset failed", map[string]interface{}{"assetType": assetType, "id": id.Hex(), "error": err})
		return pkgerrors.NewAppErrorWithCause(pkgerrors.CodeInternalError, "DeleteAsset failed", 500, err)
	}
	return nil
}
